...
try:
    import cv2
except ImportError:  # pragma: no cover - depends on optional computer vision stack
    cv2 = None
import numpy as np
import time
...
def main():
    ...
    if Tello is None or cv2 is None:
        raise TelloDependencyError(
            "djitellopy and opencv-python are required for Tello control."
        )
    tello = Tello()
